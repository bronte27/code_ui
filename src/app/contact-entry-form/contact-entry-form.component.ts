...
export class ContactEntryFormComponent implements OnInit {
  contactForm = new FormGroup({
    ...
    'emails': new FormGroup({
      'email': new FormControl('', [Validators.required, Validators.email]), // pattern('^[a-z0-9._%+-]+@[a-z0-9.-]+\\.[a-z]{2,3}$')]),
      'validateEmail': new FormControl('', [Validators.required, Validators.email]), //pattern('^[a-z0-9._%+-]+@[a-z0-9.-]+\\.[a-z]{2,3}$')])
